...
RUN apt-get -y update && apt-get install -y -q php5-cli php5-fpm php5-mysql php5-curl php5-pgsql php5-mongo php5-gd php5-intl php5-imagick php5-mcrypt php5-memcache php5-xmlrpc php5-xsl php5-dev

# Edit PHP config
RUN sed -i 's/\;date\.timezone\ \=/date\.timezone\ \=\ UTC/g' /etc/php5/fpm/php.ini && sed -i -e "s/;daemonize\s*=\s*yes/daemonize = no/g" /etc/php5/fpm/php-fpm.conf && sed -i -e "s/;listen.mode\s*=\s*0660/listen.mode = 0666/g" /etc/php5/fpm/pool.d/www.conf
...
ADD ./development/docker/supervisor/supervisord.conf /etc/supervisor/supervisord.conf
ADD ./development/docker/supervisor/conf.d /etc/supervisor/conf.d
ADD ./development/docker/nginx/nginx.conf /etc/nginx/nginx.conf
